...
if ((!empty($_POST['city_id'])) ) {
 ...
 //  $active = 'y';
	// $query = $conn->prepare("SELECT key_person_id,key_person_name,org_office_id,mobile_no1,mobile_no2,address_line3,email_id1 FROM key_person where city_id = :city_id and active=:active");
	// $query->bindParam(':city_id',$city_id);
 //  $query->bindParam(':active',$active);
  ...
  if ($key_person_row>0) {
    ...
    $query->execute();
    $key_person_result = $query->fetchAll();
    $key_person_name = $key_person_result[0]['key_person_name'];

    // echo "<pre>";
    // print_r($key_person_result);
    // echo "</pre>";
...
<div class="item">
  <div class="mb-3">
    ...
    <div class="mt-3">
      <ul>
        <li><a href="#" class="text-uppercase"><i class="fas fa-user"></i> '.$org_office_name.'</a></li>
        <li>
