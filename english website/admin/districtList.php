<?php
  ...
  $query = $conn->prepare("SELECT * From district order by id desc");
  $query->execute();
          ...
          <div class="container-fluid">
              <div class="row">
                  <div class="col-md-7">
                      <div class="d-flex">
                          <div>
                            <a href="districtAdd.php" class="btn btn-primary myNewLinkBtn">Add District</a>
                          </div>
                      </div>      
                  </div>
                                  ...
                                  if (isset($row)) {
                                    if ($row>0) {
                                      ...
                                      foreach ($result as $value) {
                                        ...
                                        $short_district_name = $value['short_district_name'];

                                        $query = $conn->prepare("SELECT * From state where id = :state_id");
                                        $query->bindParam(':state_id',$state_id);
                                        $query->execute();
                                        $stateResult = $query->fetchAll();
                                        $stateRow = count($stateResult);
                                        if($stateRow>0) {
                                          $stateName = $stateResult[0]['state_name'];
                                        }
                                        ...
                                        <tr>                                         
                                          ...
                                          <td class="fontFourteen"><?php if(!empty($stateName)){ echo $stateName; } ?></td>
                                          <td>
                                              <!-- <a href="contactFormQueryDetail.php?id=<?php if(!empty($id)){ echo $id; } ?>" class="btn far fa-eye actionView" title="View Detail"></a> -->
                                              <!-- <a href="districtEdit.php?id=<?php if(!empty($id)){ echo $id; } ?>" class="btn far fa-edit actionEdit" title="Edit Record"></a> -->
                                              <!-- <a href="backend/districtDelete.php?id=<?php if(!empty($id)){ echo $id; } ?>" onclick="return confirm('Are you sure you want to delete this record?');" class="btn far fa-trash-alt actionDelete" title="Delete Record"></a> -->
                                          </td>
                                        </tr>
                                        <?php
                                        $sr_no = $sr_no+1;
